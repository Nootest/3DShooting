...
function generateEnemies(count) {
    const bodyGeometry = new THREE.BoxGeometry(voxelSize * 1.2, voxelSize * 1.2, voxelSize * 1.2);
    // Different materials for different enemy types
    const meleeBodyMaterial = new THREE.MeshPhongMaterial({ color: 0xFF5722, shininess: 10 }); // Orange-red for melee
    const rangedBodyMaterial = new THREE.MeshPhongMaterial({ color: 0x3F51B5, shininess: 10 }); // Blue for ranged
    const eyeGeometry = new THREE.BoxGeometry(voxelSize * 0.5, voxelSize * 0.3, voxelSize * 0.1);
    ...
    while (enemiesSpawned < count) {
        ...
        // Clone appropriate material based on type
        const baseBodyMaterial = isMelee ? meleeBodyMaterial : rangedBodyMaterial;
        const bodyMaterial = baseBodyMaterial.clone(); 
        const body = new THREE.Mesh(bodyGeometry, bodyMaterial);
        body.castShadow = true;
        body.name = "enemyBody";
        const eye = new THREE.Mesh(eyeGeometry, eyeMaterial);
        eye.position.set(0, 0.2, (voxelSize * 1.2) / 2 + (voxelSize * 0.1)/2);
        enemy.add(body, eye);
        ...
        if (enemyType === 'melee') {
            ...
            enemy.baseColor = 0xFF5722; // Orange
        } else {
...
function updateProjectiles(delta) {
    // Get references to enemy bodies for raycasting
    const enemyBodies = enemies.map(e => e.children.find(c => c.name === "enemyBody")).filter(Boolean);
...
function handleEnemyHit(targetBody) {
     const targetGroup = targetBody.parent;
     ...
     // Apply damage
     targetGroup.health -= 1;

     // Visual feedback: Flash white on hit using emissive property
     // This avoids material cloning and potential memory leaks.
     if (targetBody.material && targetBody.material.emissive) {
        // Clear previous timeout if hit rapidly
        if (targetGroup.hitFlashTimeout) {
            clearTimeout(targetGroup.hitFlashTimeout);
        }

        targetBody.material.emissive.set(0xFFFFFF); // Flash white
        
        // Revert emissive color after a short delay
        targetGroup.hitFlashTimeout = setTimeout(() => {
             if (targetBody.material && targetBody.material.emissive) {
                 targetBody.material.emissive.set(0x000000); // Reset emissive
             }
     ...
     if (targetGroup.health <= 0) {
         ...
         // Dispose the cloned material to ensure no leak
         if (targetBody.material) targetBody.material.dispose(); 
...
function restartGame(attemptResume) {
    ...
    enemies.forEach(e => {
        const body = e.children.find(c => c.name === "enemyBody");
        if (body && body.material) body.material.dispose();
...
function updateEnemyPhysics(delta) {
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        // Floating animation
        enemy.position.y = 1.5 + Math.sin(time * 0.005 + enemy.position.x) * 0.2;
